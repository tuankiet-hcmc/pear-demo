...
class HlsJsLoader {
    ...
    load(context, config_unused, callbacks) {
        if (context.type) {
            this.segmentManager.loadPlaylist(context.url)
                .then((xhr) => this.successPlaylist(xhr, context, callbacks))
                .catch((error) => this.error(error, context, callbacks));
        }
        else if (context.frag) {
            this.segmentManager.loadSegment(context.url, (context.rangeStart == undefined) || (context.rangeEnd == undefined)
                ? undefined
                : { offset: context.rangeStart, length: context.rangeEnd - context.rangeStart }, (content, downloadSpeed) => setTimeout(() => this.successSegment(content, downloadSpeed, context, callbacks), 0), (error) => setTimeout(() => this.error(error, context, callbacks), 0));
        }
        else {
    ...
    successSegment(content, downloadSpeed, context, callbacks) {
        const now = performance.now();
...
class SegmentManager {
    ...
    loadPlaylist(url) {
        return __awaiter(this, void 0, void 0, function* () {
            const xhr = yield this.loadContent(url, "text");
            this.processPlaylist(url, xhr.response, xhr.responseURL);
            return xhr;
        });
    }
    loadSegment(url, byterange, onSuccess, onError) {
        ...
        if (!segmentLocation) {
            ...
            this.loadContent(url, "arraybuffer", byterangeToString(byterange))
                .then((xhr) => onSuccess(xhr.response, 0))
    ...
    loadSegments(playlist, segmentIndex, requestFirstSegment, notInPlaylistSegment) {
        const segments = [];
        ...
        if (notInPlaylistSegment) {
            ...
            segments.push(new p2p_media_loader_core_1.Segment(id, url, byterangeToString(notInPlaylistSegment.byterange), priority++));
        ...
        for (let i = segmentIndex; i < playlistSegments.length && segments.length < this.settings.forwardSegmentCount; ++i) {
            ...
            segments.push(new p2p_media_loader_core_1.Segment(id, url, byterangeToString(byterange), priority++));
...
},{"m3u8-parser":6,"p2p-media-loader-core":"p2p-media-loader-core"}],6:[function(require,module,exports){
...
function initHlsJsEvents(player, engine) {
    player.on("hlsFragChanged", function (event_unused, data) {
